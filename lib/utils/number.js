...
const DECIMAL_PATTERN = /^\s*[-+]?(0|[0-9]+\.[0-9]+)\s*$/;
const INTEGER_PATTERN = /^\s*[-+]?[0-9]+\s*$/;
...
@return {Number}    the integer part of n, or false if n is not a valid integer
*/
module.exports.getInteger = function getInteger(n) {
  var intVal = INTEGER_PATTERN.test(n) && isFinite(n) && parseInt(n, 10);
  return (intVal !== false) ? intVal : false;
};

module.exports.getDecimal = function getDecimal(n) {
  var floatVal = DECIMAL_PATTERN.test(n) && isFinite(n) && parseFloat(n, 10);
  return (floatVal !== false) || (floatVal === 0) ? floatVal : false;
};
